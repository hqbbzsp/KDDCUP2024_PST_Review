...
# import setproctitle
# setproctitle.setproctitle("zsp_python3.9")
...
def prepare_bert_input(fold = 0):
    x_train = []
    y_train = []
    x_valid = []
    y_valid = []

    splitter = SpacyTextSplitter(chunk_size=300, chunk_overlap=0)
    data_dir = join(settings.DATA_TRACE_DIR, "PST")
    papers = utils_spacy.load_json(data_dir, "paper_source_trace_train_ans.json")
    n_papers = len(papers)
    ...
    if fold == 0 :
        ...
        n_train = int(n_papers * 1 / 3)
        papers_valid = papers[:n_train]
        papers_train = papers[n_train:]

    # papers_valid = papers[n_train:2*n_train]
    # papers_train = papers[:n_train]+papers[2*n_train:]
    ...
    for cur_pid in tqdm(all_pids):
        ...
        for bid in bid_to_title:
            ...
            for label_title in source_titles:
                if fuzz.ratio(cur_ref_title, label_title) >= 90:
                    flag = True
                    cur_pos_bib.add(bid)
        ...
        bib_to_contexts = utils_spacy.find_bib_context(xml,splitter)
        ...
        for bib in cur_pos_bib:
            ...
            cur_context = " <gap> ".join(bib_to_contexts[bib])
            if cur_context:
                cur_x.append(orig_title+"\t "+cur_title+"\t "+cur_context)
                cur_y.append(1)
...
def train(year=2023, model_name="sciroberta",fold = 0):
    ...
    for epoch in trange(int(NUM_TRAIN_EPOCHS), desc="Epoch"):
        ...
        for step, batch in enumerate(progress_bar):
            ...
            if all_steps% EVAL_STEP == 0 and all_steps >= 200:
                ...
                if len(eval_ap_history) == 0 or dev_loss < min(loss_history):
                    no_improvement = 0
        ...
        if flag:
            break
    model_to_save = model.module if hasattr(model, 'module') else model
    output_model_file = os.path.join(OUTPUT_DIR, "latest_model.pth")
    torch.save(model_to_save.state_dict(), output_model_file)


def gen_kddcup_valid_submission_bert(model_name="scibert",fold = 0):
    print("model name", model_name)
    data_dir = join(settings.DATA_TRACE_DIR, "PST")
    papers = utils_spacy.load_json(data_dir, "paper_source_trace_test_wo_ans.json")

    splitter = SpacyTextSplitter(chunk_size=200, chunk_overlap=0)
    ...
    sub_example_dict = utils_spacy.load_json(data_dir, "submission_example_test.json")

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print("device", device)
    label_mask = utils_spacy.load_json(data_dir, "Btest_monogr_mask.json")

    model = RobertaForSequenceClassification.from_pretrained(BERT_MODEL, num_labels=2)
    model.load_state_dict(torch.load(join(settings.OUT_DIR, "kddcup", model_name, f"pytorch_model_{fold}.bin")))
    ...
    for paper in tqdm(papers):
        ...
        contexts_sorted = [orig_title+"\t "+bid_to_title[bib]+"\t "+" <gap> ".join(bib_to_contexts[bib]) for bib in bib_sorted]
        # print(contexts_sorted[0])
        ...
        for step, batch in enumerate(test_dataloader):
            ...
            # cur_pred_scores = [score if mas else 0.0 for mas,score in zip(mask,cur_pred_scores)]
    ...
    utils_spacy.dump_json(sub_dict, settings.RESULT_DIR, f"test_fold{fold}_sciroberta.json")

def merge_result():
    data_dir = settings.RESULT_DIR
    data_1 = utils_spacy.load_json(data_dir, "test_fold0_sciroberta.json")
    data_2 = utils_spacy.load_json(data_dir, "test_fold1_sciroberta.json")
    masks_mo = utils_spacy.load_json(join(settings.DATA_TRACE_DIR, "PST"),"Btest_monogr_mask.json")
    ...
    for key in lis:
        ...
        for idx in range(len(item_1)):
            if mask_mo[idx]:
                result_1.append(item_1[idx]*0.5+item_2[idx]*0.5)
            else :
                result_1.append(0.0)
        result[key]=result_1
    utils_spacy.dump_json(result, settings.RESULT_DIR, "roberta_spacy_fold2.json")
...
if __name__ == "__main__":
    ...
    if train_or_test == "train":
        print("------------- fold 0 start training --------------")
        prepare_bert_input(0)
        train(model_name="sciroberta",fold=0)
        print("------------- fold 1 start training --------------")
        prepare_bert_input(1)
        train(model_name="sciroberta",fold=1)

    elif train_or_test == "test":
        gen_kddcup_valid_submission_bert(model_name="sciroberta",fold=0)
        gen_kddcup_valid_submission_bert(model_name="sciroberta",fold=1)
        merge_result()
    else:
